...
static Settings load_from_config(const fs::path& path) {
    ...
    while (std::getline(in, line)) {
        ...
        static const std::unordered_map<std::string, std::string> kKeyAliases = {
            ...
            {"Visualizer", "Eliment_visualizer"}, {"AlbumArt", "Album_art"}, {"AlbumArtRadius", "Album_art_radius"}, {"DiskSize", "Disk_size"}, {"AlbumArtSmoothing", "Album_art_smoothing"}, {"AlbumArtGlyphs", "Album_art_glyphs"}, {"AlbumArtStyle", "Album_art_style"}, {"AlbumArtColor", "Album_art_color"}, {"AlbumArtTrueColor", "Album_art_truecolor"},
        ...
        if (key == "Album_art_glyphs") {
            std::string v = value;
            for (char& c : v) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
            if (v == "density") s.album_art_glyphs = 0;
            else if (v == "hue") s.album_art_glyphs = 1;
            else if (v == "uniform") s.album_art_glyphs = 2;
            continue;
        }
        ...
        if (key == "Album_art_style") {
            std::string v = value;
            for (char& c : v) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
            if (v == "braille") s.album_art_style = 0;
            else if (v == "blocks") s.album_art_style = 1;
            else if (v == "ascii") s.album_art_style = 2;
            continue;
        }
        // Accepted for configs written before AlbumArtStyle existed.
        if (key == "Album_art_color") { s.album_art_style = parse_bool(value) ? 1 : 0; continue; }
